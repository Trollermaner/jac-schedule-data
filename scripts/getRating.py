...
def getRating(id, headers):
    ...
    rating = data["data"]["node"]["avgRating"]
    numReviews = data["data"]["node"]["numRatings"]

    return [rating, numReviews]

def requestId(teacher, school_id, headers):
    json_data = {
    ...
    print(f"working on {teacher}")
    response = requests.post('https://www.ratemyprofessors.com/graphql', headers=headers, json=json_data)
    if response.status_code != 200:
        response = retry(requests.post('https://www.ratemyprofessors.com/graphql', headers=headers, json=json_data)) 
    ...
    if len(data["data"]["search"]["teachers"]["edges"]) != 0:
        ...
        if teacherMatch(first_name, last_name, school_id, teacher_node):
            ...
            ratingData = getRating(id, headers)
            teacherDict["rating"] = ratingData[0]
            teacherDict["reviews"] = ratingData[1]

            return [teacher, teacherDict]
...
def getId(teacherList, school_id):
    ...
    for teacher in teacherList:
        response =  requestId(teacher, school_id, headers)
    ...
    return json.dumps(teacherRating, indent=4)
...
start = perf_counter()
teacherRating_json = getId(teacherList, "U2Nob29sLTEyMDUw")
stop = perf_counter()
